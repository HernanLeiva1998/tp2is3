...
def pagos_json(id):
    """Retorna el json con todos 
    los pagos del socio logueado"""
    return json.dumps(pagos.listar_pagos_diccionario(id))


def pagos_adeudados_json(id):
    """Retorna el json con todos los 
    pagos adeudados del socio logueado"""
    return json.dumps(pagos.listar_pagos_adeudados_diccionario(id))


def pagar_json(json, id):
    """Recibe un json que es una lista con un 
    solo elemento que tendria datos del pago
...
@login_requerido
def pagos_socios(id):
    """Esta funcion retorna todos los 
    pagos asociados al socio solicitado"""
...
def pagar_cuota(id):
    """Paso de confirmacion antes de cambiar 
    el estado de una cuota impaga a pagada"""
...
def confirmar_pago(id):
    """Cambia el estado de una cuota de impaga a pagada. 
    Persiste la fecha y monto de pago en la base de datos"""
    if not (has_permission(session["user"], "pago_pay")):
        return abort(403)
    if (not is_integer(id)) or (pagos.get_cuota(id) is None):
        return abort(404)
